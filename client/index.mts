...
import type { BoundBulkGet } from "./impl/bulkGet.mts"
...
export function doBind(config: CouchConfigSchema) {
  ...
  const result = {
     ...
     */
    bulkGet: getBoundWithRetry<BoundBulkGet>(bulkGet, config),
    bulkGetDictionary: getBoundWithRetry<BulkGetDictionaryBound>(bulkGetDictionary, config),
    get: getBoundWithRetry<BoundGet>(get, config),
    getAtRev: getBoundWithRetry<BoundGetAtRev>(getAtRev, config),
    query: getBoundWithRetry<BoundQuery>(query, config),
